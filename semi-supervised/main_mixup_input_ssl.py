...
parser = argparse.ArgumentParser(description='Input Mixup Training')
...
parser.add_argument('--mixup_alpha_usup', type=float, default=1.0, help='alpha parameter for unsupervised mixup')
...
def adjust_learning_rate(optimizer, epoch, gammas, schedule):
    """Sets the learning rate to the initial LR decayed by gamma every 'x' epochs in schedule"""
    lr = args.lr
...
if args.dataset == 'cifar10':
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(1, args.batch_size, 2 ,'cifar10', args.data_dir, labels_per_class = 400, valid_labels_per_class =500)
    zca_components = np.load(args.data_dir +'zca_components.npy')
    zca_mean = np.load(args.data_dir +'zca_mean.npy')
if args.dataset == 'svhn':
    trainloader, validloader, unlabelledloader, testloader, num_classes = load_data_subset(1, args.batch_size, 2 ,'svhn', args.data_dir, labels_per_class = 100, valid_labels_per_class =732)
